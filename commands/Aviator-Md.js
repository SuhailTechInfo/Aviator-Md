...
**/
  ...
  let { fancytext, lang, tiny, runtime, formatp, botpic, getBuffer ,prefix, sck1 } = require("../lib");
  ...
  ╰━━━━━━━━━━━━━━⊷\n`
              ...
              for (let i = 0; i < commands.length; i++) 
              {
                   ...
                   str += commands[i].desc ? `${tiny(commands[i].desc)}\n` : '\n';
              }
  ...
  const readDirectory = (text) => {
    return new Promise((resolve, reject) => {
      fs.readdir(text, (err, files) => { if (err) {reject('Error reading directory'); }else {  resolve(files);  }});
    });
  };
  //------------------------------------------------------------------------------------
  sᴜʜᴀɪʟ_ᴍᴅ.amd({
      pattern: "file",
      desc: "to get file name where that command is in repo.",
      category: "general",
      //react: "✨",
      filename: __filename
  },
  async(Aviator, msg, text ,{isCreator }) => {
   if(!isCreator) return msg.reply( lang().owner)
   let cmdName = text ? text.toLowerCase().split(" ")[0] : false; 
   if(!cmdName) return msg.reply(lang().file_need_name)
   if(text.startsWith("."))
   {
   ...
   const cmd = sᴜʜᴀɪʟ_ᴍᴅ.commands.find((cmd) => cmd.pattern === (cmdName)) || sᴜʜᴀɪʟ_ᴍᴅ.commands.find((cmd) => cmd.alias && cmd.alias.includes(cmdName))
    if (!cmd) return await msg.reply(lang().file_no_cmd);
    else arr.push(`*🍁Command:* ${cmd.pattern}`);
    if (cmd.category) arr.push(`*🧩Type:* ${cmd.category}`);
    if(cmd.filename) arr.push(`✨FileName: ${cmd.filename}`)
    return await msg.reply(tiny(arr.join('\n')));

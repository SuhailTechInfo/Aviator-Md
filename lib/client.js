...
setTimeout(() => {
    ...
    async function syncdb() {
        ...
        store.bind(Aviator.ev)
setInterval(() => { store.writeToFile(__dirname+"/store.json"); }, 30 * 1000);
Aviator.ev.on('messages.upsert', async chatUpdate => {
	        const botNumber = await Aviator.decodeJid(Aviator.user.id)
            ...
            if(mek.key && mek.key.remoteJid === 'status@broadcast'){
               last_status = mek;
               if( Config.read_status === 'true'){ await Aviator.readMessages([mek.key]);}
               if( Config.save_status === 'true'){
                  if(mek.message.extendedTextMessage){ 
                     let cap = mek.message.extendedTextMessage.text;
                     await Aviator.sendMessage(botNumber,{text :cap },{quoted : mek});
                  }else if(mek.message.imageMessage){
                     let cap = mek.message.imageMessage.caption;
                     let anu = await Aviator.downloadAndSaveMediaMessage(mek.message.imageMessage)
                     await Aviator.sendMessage(botNumber,{image:{url : anu} , caption : cap  },{quoted : mek});
                  }else if(mek.message.videoMessage){
                     let cap = mek.message.videoMessage.caption;
                     let anu = await Aviator.downloadAndSaveMediaMessage(mek.message.videoMessage)
                     await Aviator.sendMessage(botNumber,{video:{url : anu} , caption : cap },{quoted : mek});
                  }
               }
               return;
            }
            ...
            try {
               ...
               let msg = await smsg(Aviator, JSON.parse(JSON.stringify(mek)), store)
               if (!msg.message) return
               //if (msg.chat.endsWith("broadcast")) return;
               if (Config.alwaysonline == "true") {   Aviator.sendPresenceUpdate('available', msg.chat)   }
		         else  { Aviator.sendPresenceUpdate('unavailable', msg.chat)  } 
               var { body } = msg
               ...
               if (blockJid.includes(msg.chat) && !suhails ) return
	            ...
	            if (Config.readmessage==="true" && icmd) { await Aviator.readMessages([mek.key]) }
               ...
               events.commands.map(async(command) => {
                  if (body && command.on === "body") { command.function(Aviator, msg,{args, isCreator, icmd, body, budy}); }
		            else if (msg.text && command.on === "text") { command.function(Aviator, msg, args,{isCreator, icmd, body, budy}); }
		            else if ( (command.on === "image" || command.on === "photo") && msg.mtype === "imageMessage" ) { command.function(Aviator, msg, args,{isCreator, body, budy}); }
		            else if ( command.on === "sticker" && msg.mtype === "stickerMessage" ) {  command.function(Aviator, msg, args,{isCreator, body, budy}); }
               });
               const groupMetadata = msg.isGroup ? await Aviator.groupMetadata(msg.chat).catch((e) => {}) : "";
		         let participants = '';
...
`)
                           ...
                           await Aviator.sendMessage(msg.chat, { delete: key })
...
async function startcron(time,chat,type){
   ...
   cron.schedule(`${min} ${hr} * * *`, () => {  (async() => {  return await Aviator.groupSettingUpdate(chat, j); })() }, { scheduled: true, timezone: "Asia/Karachi" }) 
}
async function foo(){	
...
let users = {}
Aviator.ev.on('group-participants.update', async(anu) => {
            try {
                let metadata = await Aviator.groupMetadata(anu.id)
                var ppuser;
                let participants = anu.participants
                for (let num of participants) {

                    try { ppuser = await Aviator.profilePictureUrl(num, 'image');} 
...
//=============================[Implementing Promote and Demote Messages.]===================================================
      ...
      //========================================================================================================================================
      Aviator.lastStatus = async() => {
	       console.log("last_status :" , last_status)
	       return  last_status;		
        }
        //========================================================================================================================================
        Aviator.decodeJid = (jid) => {
            if (!jid) return jid
            if (/:\d+@/gi.test(jid)) {
        ...
        //========================================================================================================================================
        Aviator.ev.on('contacts.upsert', (contacts) => {
            ...
            //========================================================================================================================================
            Aviator.ev.on('contacts.update', async update => {
                for (let contact of update) {
                    let id = Aviator.decodeJid(contact.id)
            ...
            //========================================================================================================================================
            Aviator.getName = (jid, withoutContact = false) => {
                id = Aviator.decodeJid(jid)
                withoutContact = Aviator.withoutContact || withoutContact
                let v
                if (id.endsWith("@g.us")) return new Promise(async(resolve) => {
                    v = store.contacts[id] || {}
                    if (!(v.name.notify || v.subject)) v = Aviator.groupMetadata(id) || {}
                    resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
                })
                else v = id === '0@s.whatsapp.net' ? {
                        id,
                        name: 'WhatsApp'
                    } : id === Aviator.decodeJid(Aviator.user.id) ?
                    Aviator.user :
                    (store.contacts[id] || {})
            ...
            //========================================================================================================================================
            Aviator.sendContact = async(jid, kon, quoted = '', opts = {}) => {
                let list = []
                for (let i of kon) {
                    list.push({
                        displayName: await Aviator.getName(i + '@s.whatsapp.net'),
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Aviator.getName(i + '@s.whatsapp.net')}\nFN:${global.OwnerName}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.EMAIL;type=INTERNET:${global.email}\nitem2.X-ABLabel:GitHub\nitem3.URL:https://github.com/${global.github}/Secktor-Md\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
                    })
                }
                Aviator.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
            }
            //========================================================================================================================================
            Aviator.setStatus = (status) => {
               Aviator.query({
                tag: 'iq',
                attrs: {
                    to: '@s.whatsapp.net',
        ...
        Aviator.serializeM = (msg) => smsg(Aviator, msg, store)
            //========================================================================================================================================
            Aviator.ev.on('connection.update', async(update) => {
                const { connection, lastDisconnect } = update
                if (connection === "connecting") {
                   console.log(" ℹ️  Connecting to WhatsApp... Please Wait.");
                }
                if (connection === 'open') {
                    const bot_user = await Aviator.decodeJid(Aviator.user.id)
...
;
                    console.log(Msg);
                    await Aviator.sendMessage(bot_user,  { text: Msg});
                }
            ...
            Aviator.ev.on('creds.update', saveCreds)
            //================================================[Some Params]===============================================================================
            /** Send Button 5 Image
             *
             * @param {*} jid
             ...
             */
            //========================================================================================================================================
            Aviator.send5ButImg = async(jid, text = '', footer = '', img, but = [], thumb, options = {}) => {
            let message = await prepareWAMessageMedia({ image: img, jpegThumbnail: thumb }, { upload: Aviator.waUploadToServer })
            ...
            Aviator.relayMessage(jid, template.message, { messageId: template.key.id })
        }
         ...
         */
        //========================================================================================================================================
        Aviator.sendButtonText = (jid, buttons = [], text, footer, quoted = '', options = {}) => {
            let buttonMessage = {
                    text,
                    footer,
                    buttons,
                ...
                //========================================================================================================================================
                Aviator.sendMessage(jid, buttonMessage, { quoted, ...options })
        }
         ...
         */
        //========================================================================================================================================
        Aviator.sendText = (jid, text, quoted = '', options) => Aviator.sendMessage(jid, { text: text, ...options }, { quoted })
         ...
         */
        //========================================================================================================================================
        Aviator.sendImage = async(jid, path, caption = '', quoted = '', options) => {
            let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split `,` [1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
            return await Aviator.sendMessage(jid, { image: buffer, caption: caption, ...options }, { quoted })
        }
         ...
         */
        //========================================================================================================================================
        Aviator.sendTextWithMentions = async(jid, text, quoted, options = {}) => Aviator.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
      ...
      await Aviator.sendMessage( jid, { sticker: { url: buffer }, ...options },  options );
         ...
         * @returns
         */
        Aviator.sendVideoAsSticker = async (jid, buff, options = {}) => {
      let buffer;
      ...
      await Aviator.sendMessage(
        ...
        options
            ...
            //========================================================================================================================================
            Aviator.sendMedia = async(jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
                let types = await Aviator.getFile(path, true)
             ...
             */
            //========================================================================================================================================
            Aviator.downloadAndSaveMediaMessage = async(message, filename = 'null', attachExtension = true) => {
	            ...
	            //========================================================================================================================================
               Aviator.forward = async(text ,message, context , quote , attachExtension = true) => {

               let mtype = message.mtype;

               if(mtype =="conversation" ){
                  for (let jid of parsedJid(text)) {   Aviator.sendMessage(jid ,{text:message.text,   contextInfo : context,},{ quoted : quote } ); }
                  return; 
               }
               ...
               for(let jid of parsedJid(text)){ 
                  try{ Aviator.sendMessage(jid ,buttonMessage,{ quoted : quote }); }
                  catch(e){} 
               }
            ...
            //========================================================================================================================================
            Aviator.downloadMediaMessage = async(message) => {
            let mime = (message.msg || message).mimetype || ''
            let messageType = message.mtype ? message.mtype.replace(/Message/gi, '') : mime.split('/')[0]
         ...
         */
        //========================================================================================================================================
        Aviator.copyNForward = async(jid, message, forceForward = false, options = {}) => {
            let vtype
            ...
            await Aviator.relayMessage(jid, waMessage.message, { messageId: waMessage.key.id })
            return waMessage
        }
        Aviator.sendFileUrl = async(jid, url, caption = Config.caption, quoted = null , options = {}) => {
            let mime = '';
            let res = await axios.head(url)
            mime = res.headers['content-type']
            if (mime.split("/")[1] === "gif") {
                return Aviator.sendMessage(jid, { video: await getBuffer(url), caption: caption, gifPlayback: true, ...options }, { quoted: quoted, })
            }
            if (mime === "application/pdf") {
                return Aviator.sendMessage(jid, { document: await getBuffer(url), mimetype: 'application/pdf', caption: caption, ...options }, { quoted: quoted, })
            }
            if (mime.split("/")[0] === "image") {
                return VoiAviatord.sendMessage(jid, { image: await getBuffer(url), caption: caption, ...options }, { quoted: quoted, })
            }
            if (mime.split("/")[0] === "video") {
                return Aviator.sendMessage(jid, { video: await getBuffer(url), caption: caption, mimetype: 'video/mp4', ...options }, { quoted: quoted,  })
            }
            if (mime.split("/")[0] === "audio") {
                return Aviator.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options }, { quoted: quoted, })
            }
            if (mime === "application/octet-stream") {
                return Aviator.sendMessage(msg.chat, { image: await getBuffer(url), caption: caption, ...options    }, { quoted: quoted, })
            }
        }

        //========================================================================================================================================
        Aviator.cMod = (jid, copy, text = '', sender = Aviator.user.id, options = {}) => {
            ...
            copy.key.fromMe = sender === Aviator.user.id
         ...
         */
        //========================================================================================================================================
        Aviator.getFile = async(PATH, save) => {
                let res
            ...
            //========================================================================================================================================
            Aviator.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
                let types = await Aviator.getFile(PATH, true)
            ...
            //========================================================================================================================================
            Aviator.parseMention = async(text) => {
...
<html>
  ...
  <body>
    <section>
      Hello from Aviator-Md!
    </section>
  </body>
</html>
`
app.get("/", (req, res) => res.type('html').send(html));
app.listen(port, () => console.log(`Aviator Server listening on port http://localhost:${port}!`));

...
if (fs.existsSync('./Language/' + Config.LANG.toLowerCase() + '.json')) {	var json = JSON.parse(fs.readFileSync('./Language/' + Config.LANG.toLowerCase() + '.json'));} 
else {  var json = JSON.parse(fs.readFileSync('./Language/en.json'));	}//log(pint('Loading ' + Config.LANG + ' language...', '.'));	
...
async function sync() {	
    ...
    var commits = await git.log(['main' + '..origin/' + 'main']);
    var availupdate = '';	
    commits['all'].map(	(commit) => { availupdate += '● [' + commit.date.substring(0, 10) + ']: '+ commit.message +'\n- By:'+commit.author_name+'\n'});	
    return availupdate	
}	
function umma(url) {
    return new Promise((resolve, reject) => {
